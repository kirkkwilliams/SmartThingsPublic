 ...
 */
definition(
	name: "Multi Speaker Notify with Custom Sound",
	namespace: "kirkwilliams",
    author: "Kirk Williams, based off of code by SmartThings",
	description: "Play a sound, file, or custom message through any of your Speakers when the mode changes or other events occur.",
...
def mainPage() {
	dynamicPage(name: "mainPage") {
		...
		section {
			input "sonos", "capability.musicPlayer", title: "On these Speaker players", required: true, multiple: true
		}
...
private loadText() {
	switch ( actionType) {
		...
		case "Custom File":
			if (message) {
				state.sound = [uri: message, duration: "10"]
			}
			else {
				state.sound = textToSpeech("You selected the custom file option but did not enter a URL in the $app.label Smart App")
			}
			break;
		default:
			state.sound = [uri: "http://s3.amazonaws.com/smartapp-media/sonos/bell1.mp3", duration: "10"]
